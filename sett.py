...
class SettMission (object):
    ...
    def __init__ (self):
        ...
        self.hw = Hardware()
        self.detect_sett(String('setcourse'),String('big'))

    def find_sett (self):
        count = 50

        self.aicontrol.drive_z (-2.0)  ##### CHANGE ME !!
    ...
    def find_sq (self, yd, zd, la): ### y_direction & z_direction & launcher
        ...
        while not rospy.is_shutdown() and not self.aicontrol.is_fail(count):
            ...
            if len(sq_data.appear) == 1 and sq_data.appear[0]:
                saw = True
                ...
                if state[2] != -3 and state[2] != -1: ###### CHANGE ME !!!
