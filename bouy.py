            ...
            while not rospy.is_shutdown() and not self.aicontrol.is_fail(count) :
                ...
                if bouy_data.appear:
                    ...
                    if self.aicontrol.is_center([bouy_data.x,bouy_data.y],-bc,bc,-bc,bc) :
                        print bouy_data
                        if bouy_data.value > 600: ### change area value
                            print 'go to bouy'
    ...
    def find_path (self):
        ...
        while not rospy.is_shutdown() and not self.aicontrol.is_fail(count) :
            if path_data.appear:
                ...
                # rad = self.aicontrol.delta_radians(self.path_pose.position.x,self.path_pose.position.y,1)
                # self.aicontrol.turn_yaw_relative(math.degrees(rad))
                self.aicontrol.drive_x(0.1*mul)
                count -= 1
        return
